---
description: '模型命名与放置位置规则：创建时指定表名和主键（默认为''id''）；指定插入字段及时间自动维护（默认true）；时间类型如logined_at需注释；模型定义支持KV和Enum；关联关系命名规范；调用注释友好；数据表设计遵循范式，常用字段与索引命名（index/unique）统一。'
lastUpdated: '2026-06-21 16:52:15'
head:
  - - meta
    - name: 'og:title'
      content: '模型'
  ...
  - - meta
    - name: 'og:description'
      content: '模型命名与放置位置规则：创建时指定表名和主键（默认为''id''）；指定插入字段及时间自动维护（默认true）；时间类型如logined_at需注释；模型定义支持KV和Enum；关联关系命名规范；调用注释友好；数据表设计遵循范式，常用字段与索引命名（index/unique）统一。'
  - - meta
    - name: 'og:url'
