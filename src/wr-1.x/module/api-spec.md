---
description: '该接口文档规范要求：请求参数按字母排序并拼接为KV字串，进行md5(md5(字串).token)加密（token为空则传空字符串）。同时说明请求地址、动作、凭据验证以及返回规范与错误码标识。'
lastUpdated: '2026-06-22 17:28:03'
head:
  - - meta
    - name: 'og:title'
      content: 'Api 文档规范 1.0'
  - - meta
    - name: 'og:type'
      content: 'article'
  - - meta
    - name: 'og:description'
      content: '该接口文档规范要求：请求参数按字母排序并拼接为KV字串，进行md5(md5(字串).token)加密（token为空则传空字符串）。同时说明请求地址、动作、凭据验证以及返回规范与错误码标识。'
  - - meta
    - name: 'og:url'
...
接口编写参考 : https://github.com/weiran-tech/system/blob/1.0/src/Http/Request/Web/ApiV1/AuthController.php
...
错误码定义文件 : https://github.com/weiran-tech/framework/blob/1.0/src/Classes/Resp.php
