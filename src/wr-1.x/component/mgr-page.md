---
description: 'MgrPage后台管理支持配置captcha_login、资源打包、开发监听及发布到项目，集成Pjax、Splash及多种页面跳转方式。提供静态表格渲染，可设定渲染ID、列定义及列操作，包含Header、Footer、图片上传等组件。动态表格支持单文件列表、添加列、筛选器、快捷操作与批量操作。'
lastUpdated: '2026-06-22 17:22:36'
head:
  - - meta
    - name: 'og:title'
  ...
  - - meta
    - name: 'og:description'
      content: 'MgrPage后台管理支持配置captcha_login、资源打包、开发监听及发布到项目，集成Pjax、Splash及多种页面跳转方式。提供静态表格渲染，可设定渲染ID、列定义及列操作，包含Header、Footer、图片上传等组件。动态表格支持单文件列表、添加列、筛选器、快捷操作与批量操作。'
  - - meta
    - name: 'og:url'
...
::: info ℹ️
配置说明 : 需要 packeton 的私有包权限, 并且在 composer 中配置如下代码
:::
...
在资源开发过程中, 我们使用如下命令进行资源监听, `webpack.mix.js` 本不存在, 需要在 `weiran/mgr-page/resources/libs/`  
