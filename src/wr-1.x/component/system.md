---
description: '该系统定义涵盖用户管理（类型、密码加密、通行证）、平台配置（验证码、跨域、演示、上传、CSRF、Cookie加密、密码提供者、位置、路由隐藏、API签名、SSO、邮箱启用）以及中间件（JWT认证、封禁、单点登录）等核心功能与组件设置。'
lastUpdated: '2026-06-21 16:51:23'
head:
  - - meta
    - name: 'og:title'
      content: 'System / 系统'
  ...
  - - meta
    - name: 'og:description'
      content: '该系统定义涵盖用户管理（类型、密码加密、通行证）、平台配置（验证码、跨域、演示、上传、CSRF、Cookie加密、密码提供者、位置、路由隐藏、API签名、SSO、邮箱启用）以及中间件（JWT认证、封禁、单点登录）等核心功能与组件设置。'
  - - meta
    - name: 'og:url'
...
是否允许通过手机号 + 验证码进行直接注册并登录, 默认开启, 可以通过 env 变量  
