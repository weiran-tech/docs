---
description: '系统初始化定义用户类型、密码加密与通行证约定。平台配置涉及跨域、上传路径、CSRF例外、密码提供商等。数据表更新采用Progress方式，类命名规范为“数据表名+改动+Progress”。中间件包含JwtAuthenticate和Ban等。'
lastUpdated: '2026-06-22 14:13:55'
head:
  - - meta
    - name: 'og:title'
      content: '系统'
  ...
  - - meta
    - name: 'og:description'
      content: '系统初始化定义用户类型、密码加密与通行证约定。平台配置涉及跨域、上传路径、CSRF例外、密码提供商等。数据表更新采用Progress方式，类命名规范为“数据表名+改动+Progress”。中间件包含JwtAuthenticate和Ban等。'
  - - meta
    - name: 'og:url'
