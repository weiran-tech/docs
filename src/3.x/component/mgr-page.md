---
description: '在资源开发中，先复制`webpack.mix.sample.js`为`webpack.mix.js`，用`mix watch`监听资源。变更后用`php artisan py-mgr-page:mix`反向复制打包文件。使用MgrPage需运行`php artisan vendor:publish --force --tag=poppy-mix`强制更新，或加入`composer.json`的scripts中自动发布。'
lastUpdated: '2026-06-22 14:14:02'
head:
  - - meta
    - name: 'og:title'
      content: '管理后台'
  ...
  - - meta
    - name: 'og:description'
      content: '在资源开发中，先复制`webpack.mix.sample.js`为`webpack.mix.js`，用`mix watch`监听资源。变更后用`php artisan py-mgr-page:mix`反向复制打包文件。使用MgrPage需运行`php artisan vendor:publish --force --tag=poppy-mix`强制更新，或加入`composer.json`的scripts中自动发布。'
  - - meta
    - name: 'og:url'
