---
description: '该API返回JSON格式，包含状态码status（如500服务器内部错误）、提示信息message（错误或成功提示）、数据data（成功时返回附加数据）。错误码含义：0成功，1失败，2凭据超时，3不存在，4错误，5参数错误，6签名错误，7无权限，99接口错误，支持自定义6位错误码。'
lastUpdated: '2026-06-22 14:31:40'
head:
  - - meta
    - name: 'og:title'
      content: 'Resp'
  ...
  - - meta
    - name: 'og:description'
      content: '该API返回JSON格式，包含状态码status（如500服务器内部错误）、提示信息message（错误或成功提示）、数据data（成功时返回附加数据）。错误码含义：0成功，1失败，2凭据超时，3不存在，4错误，5参数错误，6签名错误，7无权限，99接口错误，支持自定义6位错误码。'
  - - meta
    - name: 'og:url'
      content: 'https://weiran.tech//4.x/framework/resp.html'
---
# Resp
