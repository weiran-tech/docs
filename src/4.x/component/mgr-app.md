...
head: 
  ...
  - - meta
    - name: 'og:description'
      content: '后端需要添加包 poppy/mgr-app 当前版本是开发版, 版本是 4.0.x-dev, 因为授权返回的地址是有此包支持, 所以需要添加此扩展包mgr-app 文件内置在 mgr-page 包的 resources 目录下, 当前是属于开发过程中, 包放置在 GitHub Releasenginx 进行如下配置mgr-app.confdomain.conf如果需要自定义目录, 从源码编译并自行配置包目录即可数据准备路径是生成前端所用导航的入口文件, 告诉前端如何加载数据, 渲染页面, 文件位于 modules/{module}/configurations/'
  - - meta
    - name: 'og:url'
...
![](https://file.wulicode.com/notion/1d/1d1aa9191094aed7fcf17bbd4ef8677a.png)
